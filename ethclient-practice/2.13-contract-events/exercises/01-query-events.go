...
func main() {
	...
	if contractAddrHex == "" {
		log.Fatal("请设置环境变量 STORE_CONTRACT_ADDRESS（已部署的 Store 合约地址）")
	}
	contractAddress := common.HexToAddress(contractAddrHex)

	// 练习：构造 FilterQuery，查询该合约在指定区块范围内的日志
	// 提示：FromBlock、ToBlock 用 big.NewInt(区块号)；Addresses 为 []common.Address{contractAddress}
	// 可先获取最新区块号，再查询最近 N 个区块
	...
	// 练习：调用 client.FilterLogs 获取匹配的日志
	// logs, err := ???
	...
	// 练习：用 abi.JSON 解析 StoreABI，遍历 logs，用 UnpackIntoInterface 解码 "ItemSet" 事件并打印
	// 提示：事件结构体为 struct{ Key [32]byte; Value [32]byte }
	...
	for _, vLog := range logs {
		fmt.Println("BlockHash:", vLog.BlockHash.Hex(), "BlockNumber:", vLog.BlockNumber, "TxHash:", vLog.TxHash.Hex())
		event := struct {
			Key   [32]byte
			Value [32]byte
		}{}
		...
		fmt.Println("Key(hex):", common.Bytes2Hex(event.Key[:]), "Value(hex):", common.Bytes2Hex(event.Value[:]))
